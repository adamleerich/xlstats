    ...
    if (meth == "closed formula") {
        ...
        if (distname == "nbinom") {
            ...
            mnbinom <- function(order, size, mu) {
                ...
                prob <- size/(size + mu)
                if (order == 3) {
                  res <- size * prob^2 + 3 * prob * (1 - prob) * 
                    (size) + (1 - prob)^2 * (size + size^2)
                  res <- res * (1 - prob)/prob^3
                }
                else if (order == 4) {
                  res <- size * prob^3 + 7 * prob^2 * (1 - prob) * 
                    (size) + 6 * prob * (1 - prob)^2 * (size + 
                    size^2) + (1 - prob)^3 * (2 * size + 3 * 
                    size^2 + size^3)
        ...
        if (distname == "geom") {
            ...
            mcumulgeom <- function(order, prob) {
            ...
            mgeom <- function(order, prob) {
                ...
                if (order == 3) 
                  return(m3)
                m4 <- mcumulgeom(4, prob) + 6 * m3 + 11 * m2 - 
                  6 * m1
                if (order == 4) 
                  return(m4)
        ...
        if (distname == "beta") {
            ...
            mbeta <- function(order, shape1, shape2) {
                stot <- shape1 + shape2
                ifelse(order == 1, shape1/stot, (shape1^3 + 2 * 
                  shape1 * shape2 + shape1^2)/stot^2/(stot + 
                  1))
            }
            order <- 1:2
        }
        if (distname == "unif") {
            ...
            munif <- function(order, min, max) {
                ifelse(order == 1, (min + max)/2, (max^2 + min^2 + 
                  max * min)/3)
            }
            order <- 1:2
        }
        if (distname == "logis") {
            ...
            mlogis <- function(order, location, scale) {
                ifelse(order == 1, location, scale^2 * pi^2/3 + 
                  location^2)
            }
            order <- 1:2
        }
        ...
        if (meth == "closed formula") {
            ...
            else memp <- function(x, order, weights) sum(x^order * 
                weights)/sum(weights)
        }
        if (calcvcov) {
            if (distname == "norm") {
                varcovar <- mme.vcov(estimate, NULL, order, data, 
                  mnorm, memp, weights)
            }
        ...
        res <- list(estimate = estimate, convergence = 0, value = NULL, 
            ...
            method = meth, order = order, memp = NULL, vcov = varcovar)
    }
    else {
        if (is.vector(start)) 
            start <- as.list(start)
        ...
        if (is.null(weights)) {
            ...
            fnobj <- function(par, fix.arg, obs, mdistnam, memp, 
                weights) mean(sapply(order, function(o) DIFF2(par, 
        ...
        if (is.null(custom.optim)) {
            ...
            if (calcvcov) {
                varcovar <- mme.vcov(opt$par, fix.arg, order, 
                  data, mdistname, memp, weights)
                if (!is.null(varcovar)) 
            ...
            res <- list(estimate = opt$par, convergence = opt$convergence, 
                ...
                order = order, memp = memp, vcov = varcovar)
        }
        else {
            opt.meth <- NULL
            if (!cens) {
